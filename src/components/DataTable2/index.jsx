          ...
          {safeRows.length ? (
            safeRows.map((r, i) => (
              <div key={i} className="bg-gray-50/50 rounded-lg p-3 border border-gray-100">
                {columns.map((c) => (
                  <div key={c.key} className="flex justify-between items-start py-1 border-b border-gray-100 last:border-b-0">
                    <span className="text-xs font-medium text-gray-600 w-1/3">{c.header}:</span>
                    <span className="text-xs text-gray-900 w-2/3 text-right">
        ...
        <div className="overflow-x-auto overflow-y-auto h-[300px]">
          <table className="min-w-full">
            <thead className="bg-gray-50/50 border-b border-gray-100 sticky top-0">
              <tr>
                {columns.map((c) => (
                  <th key={c.key} className="text-left bg-white px-4 py-3 text-xs font-medium text-gray-900">
                    {c.header}
                  </th>
                ))}
              </tr>
              ...
              {safeRows.length ? (
                ...
                <tr>
                  <td
                    ...
                    className="px-4 py-8 text-gray-500 text-center text-xs"
                  >
                    No data available
                  </td>
                </tr>
